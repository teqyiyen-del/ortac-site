...
/* ============================================================================
   ...
   page.tsx sunucu bileşeni kalmak ZORUNDA: `export const metadata` bir istemci
   dosyasından verilemiyor. Native <title>/<meta> ile denendi ve sonuç kötüydü —
   layout.tsx'in varsayılan başlığı da basıldığı için <head> içinde İKİ <title>
   ve İKİ description oluşuyor. Bu yüzden sayfa ikiye bölündü: metadata, JSON-LD
   ve kabuk page.tsx'te (sunucu), durum tutan her şey burada (istemci).

   ---------------------------------------------------------------------------
   AD ALANI: .ct- (lab kopyası .i6- KALIYOR)

   Lab adayı src/components/lab/ContactI6.tsx ve src/app/css/lab-i6.css karar
   kaydı olarak duruyor ve lab-i6.css hâlâ globals.css'e import ediliyor. Canlı
   kopya bu yüzden .i6- DEĞİL .ct- kullanıyor: aynı seçici adını iki dosyada
   kullanmak, sonra import edilenin diğerini sessizce ezmesi demek — bu depoda
   daha önce yaşandı (bkz. lab-i6.css'teki .i6-mlbl notu). Sınıflarla birlikte
   id'ler ve keyframe adı da .ct-/ct- önekine taşındı.

   ---------------------------------------------------------------------------
   LAB'DEN GELEN VE KORUNAN KARARLAR

   · Üç ofis omurgası: üç büyük ofis düğmesi → tam genişlikte gerçek harita →
     üç büyük kanal kartı. Sayfanın ilk kararı "hangi kanaldan yazayım" değil,
     "hangi ofisle konuşuyorum".
   · Harita gerçek: Natural Earth 110m kıyı çizgisi, derleme dışında bir kez
     üretilip bu dosyaya gömülü. Çalışma anında dış istek, harita karosu ya da
     API anahtarı YOK.
   · Formda tek bir <select> yok — ülke ve konu, gizli yerli radyo düğmesinin
     üstüne çizilmiş kutucuklar. Ok tuşuyla gezinme ve ekran okuyucu duyurusu
     tarayıcıdan geliyor, taklit edilmiyor.
   · Website alanı var (type="url", isteğe bağlı).
   · Ülke başına saat kadranı, hizmet başına muhatap, müşteri paneli ve
     "bunları beklemeyin" bandı YOK. Firmada karşılığı olmayan yapılar.
   · Konu listesi servicesFor()'dan türüyor, elle yazılmıyor.

   ---------------------------------------------------------------------------
   LAB'DEN FARKLI OLAN İKİ ŞEY

   1) BAŞLIK. Lab kopyası kendi H1'ini basıyordu. Sayfa olarak PageHero
...
/* =============================================================== 1 · OFİSLER
   Ofis seçici, harita ve büyük kanal kartları. Hepsi tek bölümde, çünkü hepsi
   tek bir seçime bağlı: seçiciyi görmeden değişen bir kart, ziyaretçiye neyin
   neyi değiştirdiğini anlatmıyor. */
...
function Offices({ active, onPick }: { active: Country; onPick: (c: Country) => void }) {
  ...
  return (
    <>
      ...
      <FadeUp delay={0.06}>
        <div className="ct-switch" role="group" aria-label="Hangi ofis">
          {OFFICES.map((o) => (
            <button
              key={o.country}
              type="button"
            ...
            >
              ...
              <span className="ct-office-t">{o.label} ofisi</span>
              <span className="ct-office-mark" aria-hidden="true">
        ...
        <div className="ct-map">
          ...
          >
            ...
            >
              ...
              <circle className="ct-ring-c" r="22" />
            </motion.g>
              ...
              {OFFICES.map((o) => {
                ...
                return (
                  ...
                  >
                    ...
                    <circle className="ct-pin-o" cx={p.x} cy={p.y} r="9" />
                    <circle className="ct-pin-i" cx={p.x} cy={p.y} r="4" />
                    {/* .ct-mlbl, .ct-label DEĞİL: form alanı etiketleri de
                        .ct-label kullanıyor ve iki kural birbirini eziyordu —
          ...
          {/* ---- haritanın üstündeki ofis kartı ----
              ...
              okunmaz yapardı. */}
          <div className="ct-card" aria-live="polite">
            <div className="ct-card-top">
              <span className="ct-card-flag" aria-hidden="true">
                <Flag country={office.country} />
              </span>
              <b className="ct-card-t">
                {office.label} ofisi
            ...
            {office.legal ? <p className="ct-card-legal">{office.legal}</p> : null}

            <p className="ct-card-addr">
      ...
      <div className="ct-chs">
        {CHANNELS.map((c, i) => {
          ...
          const body = (
            <>
              <span className="ct-ch-ic" aria-hidden="true">
          ...
          return (
            <FadeUp key={c.kind} delay={0.06 * i} className="ct-ch-w">
...
/* BAŞLANGIÇ SEÇİMİ BOŞ DEĞİL — Dubai + şirket kuruluşu seçili geliyor.

   Müşterinin kararı: "form kısmında şuan hiçbir şey tıklı değil de bence
   dubai ve şirket kuruluşu placeholder olarak tıklanık gelsin. en üstte cümle
   de belirmiş olur hem güzel durur."

   İki işe birden yarıyor: formun üstündeki canlı cümle boş bir iskelet yerine
   kurulmuş bir cümleyle açılıyor, ve en sık gelen talep zaten bu ikisi —
   ziyaretçinin çoğu tek tıkla değil sıfır tıkla doğru yerde başlıyor.

   Seçim ön işaretli ama KİLİTLİ DEĞİL: iki grup da normal radyo, değiştirmek
   tek tık. Zorunluluk denetimi de değişmiyor (REQUIRED aynı) — sadece alanlar
   geçerli bir değerle başlıyor.

   Değerler elle yazılmıyor: kuruluşun slug'ı services.ts'ten (FORMATION_SLUG)
   geliyor, yani orada değişirse burası kendiliğinden doğru kalıyor. */
const EMPTY: Values = {
  ulke: "dubai",
  hizmet: FORMATION_SLUG,
  ad: "",
  eposta: "",
  telefon: "",
  website: "",
...
function ContactForm() {
  ...
  /* --- canlı cümle ---
     Ziyaretçi ne göndereceğini formu bitirmeden görüyor. Cümle kademeli
     kuruluyor. Dört hâl ayrı ayrı yazıldı çünkü tek bir şablona sıkıştırmak
     Türkçeyi bozuyordu: "birden fazla konu konusunda" ve "Ülkeye karar
     vermeden için" ikisi de o şablonun ürünüydü. */
  const ulkeToken =
    values.ulke === ""
      ? null
  ...
  if (!ulkeToken && !hizmetToken) {
    ...
    say = (
      <>
        <b className="ct-tok">{ulkeToken}</b>
    ...
    say = (
      <>
        <b className="ct-tok">{hizmetToken}</b>
        {konuKuyruk}
      </>
    );
  } else {
  ...
  return (
    ...
    >
      ...
      <p className="ct-say" aria-live="polite">
        {say}
      </p>
      ...
      <fieldset className="ct-block" aria-describedby={shown("ulke") ? "ct-ulke-err" : undefined}>
        <legend className="ct-legend">
          Hangi ülke?
          <b className="ct-req" aria-hidden="true">
            *
          </b>
          ...
          {COUNTRY_OPTIONS.map((o) => (
            ...
            >
              <input
                ...
                onBlur={() => blur("ulke")}
              />
              <span className="ct-ctry-ic" aria-hidden="true">
                {o.country ? <Flag country={o.country} /> : <Compass size={18} strokeWidth={1.9} />}
              </span>
              <span className="ct-ctry-t">{o.label}</span>
            </label>
          ))}
        </div>

        {shown("ulke") ? (
          <p className="ct-err" id="ct-ulke-err" role="alert">
            {shown("ulke")}
          </p>
        ) : null}
      </fieldset>
      ...
      <fieldset
        className="ct-block"
        aria-describedby={shown("hizmet") ? "ct-hizmet-err ct-hizmet-hint" : "ct-hizmet-hint"}
      >
        <legend className="ct-legend">
          Hangi konuda?
          <b className="ct-req" aria-hidden="true">
        ...
        <p className="ct-hint" id="ct-hizmet-hint">
        ...
        {dropped ? (
          <p className="ct-drop" role="status">
        ...
        {shown("hizmet") ? (
          <p className="ct-err" id="ct-hizmet-err" role="alert">
            {shown("hizmet")}
          </p>
        ) : null}
      </fieldset>
      ...
      {/* ================= BİLGİLERİNİZ ================================= */}
      <fieldset className="ct-block">
        <legend className="ct-legend">Size nasıl dönelim?</legend>

        <div className="ct-fields">
          <div className="ct-field" data-bad={shown("ad") ? "" : undefined}>
            <label className="ct-label" htmlFor="ct-ad">
              Ad Soyad
              <b className="ct-req" aria-hidden="true">
                *
              </b>
            ...
            <input
              ...
              aria-describedby={shown("ad") ? "ct-ad-err" : undefined}
              onChange={(e) => set("ad", e.target.value)}
              onBlur={() => blur("ad")}
            />
            {shown("ad") ? (
              <p className="ct-err" id="ct-ad-err" role="alert">
                {shown("ad")}
              </p>
            ) : null}
          </div>

          <div className="ct-field" data-bad={shown("eposta") ? "" : undefined}>
            <label className="ct-label" htmlFor="ct-eposta">
              E-posta
              <b className="ct-req" aria-hidden="true">
                *
              </b>
            ...
            <input
              ...
              aria-describedby={shown("eposta") ? "ct-eposta-err" : undefined}
              onChange={(e) => set("eposta", e.target.value)}
              onBlur={() => blur("eposta")}
            />
            {shown("eposta") ? (
              <p className="ct-err" id="ct-eposta-err" role="alert">
                {shown("eposta")}
              </p>
            ) : null}
          </div>
          ...
          <div className="ct-field" data-wide="" data-bad={shown("mesaj") ? "" : undefined}>
            <label className="ct-label" htmlFor="ct-mesaj">
              Mesajınız
              <b className="ct-req" aria-hidden="true">
                *
              </b>
            ...
            <textarea
              ...
              aria-describedby={shown("mesaj") ? "ct-mesaj-err" : "ct-mesaj-hint"}
              onChange={(e) => set("mesaj", e.target.value)}
              onBlur={() => blur("mesaj")}
            />
            {shown("mesaj") ? (
              <p className="ct-err" id="ct-mesaj-err" role="alert">
                {shown("mesaj")}
              </p>
            ) : (
