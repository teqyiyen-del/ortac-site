/* Marka işaretleri — tek kayıt defteri.
 *
 * Sorun: site "Stripe, PayPal ve Wam ile tahsilat" diyor ama ekranda hiçbir
 * marka görünmüyordu. Her şey soyut vektöre dönünce iddia da soyut kalıyor;
 * ziyaretçi tanıdığı bir logoyu görmeden "bu gerçekten çalışıyor" demiyor.
 *
 * İki tür işaret var:
 *
 * 1. `path` taşıyanlar — küresel markalar. Yollar simple-icons'tan (CC0-1.0)
 *    alınıp buraya gömüldü; paket bağımlılık olarak durmuyor. Hepsi 24×24
 *    viewBox'ta tek `path`, tek renk. Markalar sahiplerinin tescilli
 *    işaretleridir; burada yalnızca gerçekten kullandığımız altyapıyı
 *    göstermek için basılıyorlar.
 *
 * 2. `mono` taşıyanlar — SWAP:BRAND_ASSET. Wio, Mashreq, IFZA ve Wam'in resmî
 *    vektörü elimizde yok. Renk uydurmuyoruz: bunlar markanın baş harfiyle
 *    nötr bir kilit olarak çıkıyor. Resmî SVG geldiğinde tek yapılacak şey
 *    ilgili satıra `path` ve `hex` eklemek — çağrı yerlerinin hiçbiri
 *    değişmiyor.
 */

export type Brand = {
  title: string;
  /** 24×24 viewBox'ta tek yol; yoksa monogram basılır */
  path?: string;
  /** markanın kendi rengi; yalnızca `path` ile birlikte anlamlı */
  hex?: string;
...
export const BRANDS = {
  ...
  payoneer: {
    title: "Payoneer",
    hex: "#FF4800",
    path: "M1.474 3.31c.234 1.802 1.035 5.642 1.398 7.263.095.459.201.853.298 1.013.501.865.907-.287.907-.287C5.644 6.616 3.17 3.597 2.38 2.787c-.139-.15-.384-.332-.608-.396-.32-.095-.374.086-.374.236.01.148.065.565.075.682zm21.835-1.463c.31.224 1.386 1.355 0 1.526-1.984.234-5.76.373-12.022 5.61C8.92 10.968 3.607 16.311.76 22.957a.181.181 0 01-.216.106c-.255-.074-.714-.352-.48-1.418.32-1.44 3.201-8.938 10.817-15.552 2.485-2.155 8.416-7.232 12.426-4.245z",
  },
  visa: {
    title: "Visa",
    hex: "#1A1F71",
  ...
  /* SWAP:BRAND_ASSET — resmî vektör bekleyenler */
  wio: { title: "Wio Business", mono: "W" },
  mashreq: { title: "Mashreq NeoBiz", mono: "M" },
  ifza: { title: "IFZA", mono: "IF" },
  wam: { title: "Wam", mono: "Wa" },
